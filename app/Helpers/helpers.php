 ...
 * @return string
 */
function formatedSize($bytes, int $precision = 1): string
{
...
function getMediaColumn($mediaModel, $mediaCollectionName = '', $optionClass = '', $mediaThumbnail = 'icon'): string
{
 ...
 */
function getDateColumn($modelObject, string $attributeName = 'updated_at'): array|string|null
{
 ...
 * @return string
 */
function getColorColumn($column, $attributeName): string
{
    if (isset($column)) {
 ...
 * @return string
 */
function getStripedHtmlColumn($column, $attributeName, $limit = 40): string
{
    if (isset($column)) {
        if ($limit == 0) {
 ...
 * @return string
 */
function getPayment($column, $attributeName): string
{
 ...
 * @return string
 */
function getLinksColumn(array|Collection $array = [], string $baseUrl = '', string $idAttribute = 'id', string $titleAttribute = 'title'): string
{
...
function getArrayColumn($array = [], $titleAttribute = 'title', $optionClass = '', $separator = ', '): string
{
    $result = [];
    foreach ($array as $link) {
...
function getEmailColumn($column, $attributeName): string
{
    if (isset($column)) {
        if ($column[$attributeName]) {
...
function generateCustomField($fields, $fieldsValues = null): string
{
    $htmlFields = [];
 ...
 * @throws FatalError
 */
function render($__php, $__data = null): string
{
 ...
 * @return array
 */
function getCustomFieldsValues($customFields = null, $request = null): array
{
    ...
    foreach ($customFields as $cf) {
        ...
        $customFieldsValues[] = ['custom_field_id' => $cf->id, 'value' => $value, 'view' => $view];
    }
 ...
 * ]
 */
function convertToAssoc($collection, $key): array
{
    $newCollection = [];
 ...
 * @return mixed|string
 */
function getOnlyClassName($fullClassName, bool $isSnake = true): mixed
{
...
function getModelsClasses(string $dir, array $excepts = null): array
{
    if ($excepts === null) {
        $excepts = ['App\Models\Upload', 'App\Models\CustomField', 'App\Models\Media', 'App\Models\CustomFieldValue',];
    }
...
function getNeededArray($delimiter = '|', $string = '', $input = ''): array
{
